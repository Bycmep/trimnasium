...
/*  the default value is false unless specified
    ...
    noTrim: don't trim the whitespace */
        ...
        let dict, resizer, focusblock; this.value = "";
        element.contentEditable = true; 

        this.#placeholder();
        element.style.cursor = "pointer";
        element.addEventListener("focusin", start);
        element.addEventListener("focusout", finish);
        ...
        function finish(e) {
            if (obj.dict_active) { e.preventDefault(); return; }
            if (dict) {
                resizer.disconnect();
        ...
        function paste(e) {
            e.preventDefault();
            let to_paste = e.clipboardData.getData('text/plain');
            if (obj.hardlimit && to_paste.length > obj.hardlimit)
                to_paste = to_paste.substring(0, obj.hardlimit);
            to_paste = obj.filter(to_paste);
            if (to_paste == "") return;
    ...
    set(s) {
        this.value = this.filter(s).substring(0, this.maxlength); this.e.innerText = this.value;
        this.#placeholder();
    }
