        ...
        const createElement = _ => {
            ...
            if (content[TAG][0] === "'" || content[TAG][0] === '"') {
                ...
                if (["checkbox", "radio", "text", "number", "password", "url"].includes(tag0)) {
                    tag[0] = "input";
                    const type = `type:${tag0}`; const params = content[PARAM];
                    content[PARAM] = params? `${params} ${type}` : type;
                    content[TAG] = tag.join(".");
                }
                obj = parent.insert(content[TAG]);
            }
            content[TAG] = "";
            ...
            if (content[PARAM]) {
                ...
                const pair = [ "", "" ];
                let i = 0;
                const add_pair = _ => { if (pair[0]) map[pair[0]] = unquote(pair[1]); pair[0] = ""; pair[1] = ""; i = 0; };
