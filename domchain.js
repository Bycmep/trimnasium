export class Page {
    ...
    constructor(type, svg) {
        if (typeof(type) === "string") {
            ...
            this.#svg = svg || tag[0].toLowerCase() === 'svg';
            if (tag[0].toLowerCase() === 'text' && !this.#svg) {
                this.e = document.createTextNode(""); return;
            } 
            this.e = (this.#svg)? 
                document.createElementNS("http://www.w3.org/2000/svg", tag[0]) :
                document.createElement(tag[0]);
            for (let i = 1; i < tag.length; i++) this.e.classList.add(tag[i]);
            return;
        }
        this.e = type;
        this.#svg = svg || type.tagName.toLowerCase() === 'svg';
    }
    ...
    static #create(type, parent, properties) {
        ...
        const created = new Page(type, parent.#svg); created.parent = parent;
        if (created.e.nodeType === Node.TEXT_NODE) created.e.nodeValue = properties;
        else created.set(properties);
        return created;
    }
    ...
    replace(object) {
        ...
        const index = this.parent.child.indexOf(this);
        this.parent.child[index] = object;
        const id = this.e.id;
        this.e.replaceWith(object.e);
        if (id) object.id(id);
        return object;
    }
    ...
    static setRoot(properties) {
        const root = document.documentElement.style;
        for (const p in properties) root.setProperty(`--${p}`, properties[p]);
    }
    ...
    text(value) { 
        ...
        else if (this.e.nodeType === Node.TEXT_NODE) this.e.nodeValue = value;
    ...
    getaBox() {
        const box = this.e.getBoundingClientRect();
        return { left: box.left + window.scrollX, x: box.x + window.scrollX, right: box.right + window.scrollX,
            top: box.top + window.scrollY, y: box.y + window.scrollY, bottom: box.bottom + window.scrollY,
